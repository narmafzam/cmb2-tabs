...
class Tab
{
    ...
    {
        wp_register_script('cmb-tabs', $this->getPathUrl('/js/tabs.js'), array('jquery'), self::VERSION, true);
        wp_enqueue_script('cmb-tabs');

        wp_enqueue_style('cmb-tabs', $this->getPathUrl('/css/tabs.css'), array(), self::VERSION);
        wp_enqueue_style('cmb-tabs');
    }

    public function setupDarkMode()
    {
        wp_enqueue_style('cmb-tabs-dark-mode', $this->getPathUrl('/css/dark-mode.css'), array(), self::VERSION);
